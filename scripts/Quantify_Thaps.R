...
df_filter <- df_all %>%
  ...
  mutate(t_MAX = replace_na(t_MAX, 480))
...
make_plot <- function(data, variable, y_label) {
  ggplot(data, aes(x = Genotype, y = .data[[variable]], fill = Genotype)) +
    ...
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.05,
                 position = position_dodge(width = 0.8)) +
    geom_point(aes(shape = Day, color = Day), size = 3,
    ...
    scale_shape_manual(values = c("Day1" = 15, "Day2" = 20)) +
    scale_color_manual(values = c("Day1" = "blue", "Day2" = "blue")) +
