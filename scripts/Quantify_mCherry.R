...
write.csv(merged_df, "20250703_Hek_RyR3_CISD2_mCherry_QR.csv", row.names=FALSE)
...
make_plot <- function(data, variable, y_label) {
  ggplot(data, aes(x = Genotype, y = .data[[variable]], fill = Genotype)) +
    stat_summary(fun = mean, geom = "bar", color = "black", width = 0.4,
                 position = position_dodge(width = 0.8)) +
    stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.05,
                 position = position_dodge(width = 0.8)) +
    geom_point(aes(shape = Day), size = 2.5,
               position = position_jitterdodge(jitter.width = 0.0, dodge.width = 0.4)) +
    labs(title = paste(y_label, "by Genotype"),
         y = y_label, x = "") +
    theme_minimal() +
    scale_shape_manual(values = setNames(16:(16 + nlevels(data$Day) - 1), levels(data$Day))) +
    scale_fill_manual(values = c("RyR3_WT" = "grey", "RyR3_Empty" = "green4","RyR3_CISD2_CatDead" = "blue", "RyR3_CISD2_Mutated" = "purple")) +
    theme(legend.position = "right")
}

# Not normalized Bar Plots
p1 <- make_plot(df_summary_clean, "Max_amplitude", "Max Amplitude")
print(p1)

p2 <- make_plot(df_summary_clean, "AUC", "AUC")
print(p2)

p3 <- make_plot(df_summary_clean, "Basal", "Basal")
print(p3)

p4 <- make_plot(df_summary_clean, "t_MAX", "t_MAX")
print(p4)
